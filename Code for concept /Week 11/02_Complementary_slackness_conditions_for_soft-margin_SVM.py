...
# Get the Lagrange multipliers (alphas) and support vectors
...
decision_values = np.dot(svm.coef_, X_train.T) + bias

# Compute the slack variables
slack_variables = 1 - y_train * decision_values

# Check complementary slackness conditions
for i, alpha in enumerate(alphas):
    if alpha > 0 and slack_variables[i] > 0:
        print("Complementary slackness condition not satisfied for sample", i)
...
#We extract the Lagrange multipliers (alphas)
